// app/app.js - Versión Final y Definitiva CORREGIDA

// --- Funciones de carga dinámica ---

async function loadGradeData(gradeKey) {
  try {
    // RUTA CORREGIDA: ../data/ (subir un nivel desde app/)
    ...
    // Normalizar estructura: si es objeto con .themes, usarlo; si es array, usarlo directamente
    let themesArray = [];
    
    if (Array.isArray(data)) {
      themesArray = data;
    } else if (data && typeof data === 'object' && Array.isArray(data.themes)) {
    ...
    console.log(`📦 Número de temas cargados: ${themesArray.length}`);
    return themesArray;
  } catch (error) {
...
async function generateMarkdownPreview() {
  ...
  try {
    ...
    if (Array.isArray(rawGrade)) {
      themesArray = rawGrade;
    } else if (rawGrade && typeof rawGrade === 'object' && Array.isArray(rawGrade.themes)) {
      themesArray = rawGrade.themes;
    } else if (rawGrade && typeof rawGrade === 'object' && Array.isArray(rawGrade.data)) {
      themesArray = rawGrade.data;
    } else {
      throw new Error("Estructura de datos no reconocida");
    }
    
    const themeObj = themesArray.find(t => t.theme.id === themeId);
...
async function exportThemeDocx() {
  ...
  try {
    const { enrichTheme } = await import('./enrichment.js');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Conectar botones - IMPORTANTE: IDs cambiados en index.html
  document.getElementById('genPreview')?.addEventListener('click', generateMarkdownPreview);
  document.getElementById('exportThemeDOC')?.addEventListener('click', exportThemeDocx); // ID corregido
  document.getElementById('exportLessonDOC')?.addEventListener('click', exportLessonsDocx); // ID corregido
  
  // Conectar botones de PDF (funciones por implementar)
  document.getElementById('exportThemePDF')?.addEventListener('click', () => {
    alert("Exportación a PDF aún no implementada");
  });
  document.getElementById('exportLessonPDF')?.addEventListener('click', () => {
    alert("Exportación a PDF aún no implementada");
  });
  ...
  // Conectar otros botones de configuración
  document.getElementById('saveCfg')?.addEventListener('click', () => {
    alert("Función guardar configuración no implementada");
  });
  
  document.getElementById('loadCfg')?.addEventListener('click', () => {
    alert("Función cargar configuración no implementada");
  });
  
  document.getElementById('clearSW')?.addEventListener('click', () => {
    alert("Función borrar caché del Service Worker no implementada");
  });
  
  // Agregar diagnóstico de rutas
  console.log("🧪 Diagnóstico de rutas:");
  console.log("📍 Ubicación actual:", window.location.href);
  console.log("📁 app.js cargado desde: app/app.js");
