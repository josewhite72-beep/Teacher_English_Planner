...
function extractScenarios(data) {
  ...
  data.forEach((item, index) => {
    ...
    if (item.scenario && typeof item.scenario === 'object') {
      ...
      // CORREGIDO: Se usa un nombre más descriptivo si no hay escenario
      scenarioId = `scenario_${index}`;
      scenarioName = item.name || item.title || `Escenario ${index + 1}`;
    }
    ...
    const themeName = (currentLang === 'es' ? item.theme?.name_es : item.theme?.name_en)
      || item.theme?.name_en || item.theme?.name_es || `Tema ${index + 1}`;
...
function populateThemes(scenarioId) {
  ...
  scenario.themes.forEach(theme => {
    ...
    // CORREGIDO: Se asegura que el nombre del tema se use correctamente
    option.textContent = theme.name || `Tema ${theme.id}`;
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      docChildren.push(
        ...
        new Paragraph({
          children: [new TextRun(currentLang === 'es' ? "ETAPAS DE LA LECCIÓN" : "LESSON STAGES", bold: true )],
          heading: HeadingLevel.HEADING_2,
        }),
...
async function exportThemePDF() {
  try {
        ...
        <html>
          ...
          <body>
            ...
            <div class="section">
              ...
              ${Object.entries(planData.rawData.standards || {}).map(([k, v]) => `<p><strong>${k}:</strong> ${Array.isArray(v) ? v.join(', ') : v || 'N/A'}</p>`).join('')}
            </div>
            <div class="section">
              <h2>${t('learningOutcomesTitle')}</h2>
              ${Object.entries(planData.rawData.learning_outcomes || {}).map(([k, v]) => `<p><strong>${k}:</strong> ${v || 'N/A'}</p>`).join('')}
            </div>
            <div class="section">
              <h2>${t('communicativeCompetencesTitle')}</h2>
              ${Object.entries(planData.rawData.communicative_competences || {}).map(([k, v]) => `<p><strong>${k}:</strong> ${typeof v === 'object' ? JSON.stringify(v) : v || 'N/A'}</p>`).join('')}
            </div>
            <div class="section">
              <h2>${t('assessmentIdeasTitle')}</h2>
              ${planData.rawData.assessment_ideas?.formative?.map(idea => `<p><strong>${t('formative')}:</strong> ${idea}</p>`).join('')}
              ${planData.rawData.assessment_ideas?.summative?.map(idea => `<p><strong>${t('summative')}:</strong> ${idea}</p>`).join('')}
            </div>
    ...
    let y = 100;
    doc.setFontSize(14);
    doc.text(t('standardsTitle'), 20, y);
    y += 10;
    for (const [key, value] of Object.entries(planData.rawData.standards || {})) {
      doc.text(`${key}: ${Array.isArray(value) ? value.join(', ') : value || 'N/A'}`, 20, y);
      y += 10;
    }

    y += 10;
    doc.text(t('learningOutcomesTitle'), 20, y);
    y += 10;
    ...
    y += 10;
    doc.text(t('communicativeCompetencesTitle'), 20, y);
    y += 10;
    for (const [key, value] of Object.entries(planData.rawData.communicative_competences || {})) {
      doc.text(`${key}: ${typeof value === 'object' ? JSON.stringify(value) : value || 'N/A'}`, 20, y);
      y += 10;
    }

    y += 10;
    doc.text(t('assessmentIdeasTitle'), 20, y);
    y += 10;
    if (planData.rawData.assessment_ideas) {
      for (const [key, value] of Object.entries(planData.rawData.assessment_ideas)) {
        if (Array.isArray(value)) {
          value.forEach(idea => {
            doc.text(`${key === 'formative' ? t('formative') : t('summative')}: ${idea}`, 20, y);
            y += 10;
          });
        }
      }
